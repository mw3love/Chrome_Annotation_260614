...
- [x] **21. 접힘 상태 주석 정리 패널 짤림 수정 (0.13.1)** — 도구막대를 접은 채 Alt+F3(주석 정리 단축키)로 패널을 열면, 우상단 손잡이의 **좌변 기준**으로 360px 패널이 펼쳐져 오른쪽이 화면 밖으로 삐져나가 짤리던 버그. 원인: `applyPanelPos`가 접힘일 땐 좌측 정렬을 갱신하지 않고 stale `panelPos.left`(우상단 손잡이 근처)를 그대로 써서, 좁아진 손잡이 좌변에 넓은 패널 좌변을 맞춤. → 접힘 분기 추가: 패널 **우변을 손잡이 우변(`br.right`)에 맞추고** `Math.max(8, br.right - panel.offsetWidth)`로 화면 안 clamp. 펼침은 기존 좌측 정렬 분기로 정상 복원.
  - 검증: **사용자 실조건 확인(2026-06-27, "잘됨")** — 접힘+Alt+F3 시 패널이 화면 안에 들어옴.
